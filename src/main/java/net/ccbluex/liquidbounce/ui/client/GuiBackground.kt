...
class GuiBackground(val prevGui: GuiScreen) : GuiScreen() {
    ...
    override fun initGui() {
        ...
        buttonList.add(GuiButton(3, width / 2 - 100, height / 4 + 40 + 25 * 5, 98, 20, "Change"))
        buttonList.add(GuiButton(4, width / 2 + 2, height / 4 + 40 + 25 * 5, 98, 20, "Reset"))
    ...
    private fun updateButtons() {
        ...
        blurButton.displayString = "Background Blur (${if (blur) "On" else "Off"})"
        typeButton.displayString = "GradientType: ${GradientBackground.gradientSide}"
        particlesButton.displayString = "Particles (${if (particles) "On" else "Off"})"
        animatedButton.displayString = "GradientAnimType (${if (GradientBackground.animated) "On" else "Off"})"
